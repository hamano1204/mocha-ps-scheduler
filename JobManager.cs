...
public class JobManager(NotificationManager notificationManager)
{
    ...
    {
        return _runningJobs.TryGetValue(jobId, out var cts) && cts.IsCancellationRequested;
    }
    ...
    private async Task ExecuteJobWithRetryAsync(JobConfig job, CancellationTokenSource cts, bool isManual)
    {
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++)
        {
            ...
            try
            {
                runner = JobRunnerFactory.CreateRunner(job.Type);
            }
            catch (Exception ex)
            {
                LogManager.LogApp($"Failed to create runner for job '{job.Id}': {ex.Message}", "ERROR");
                LogManager.LogJob(job.Id, $"Runner creation failed: {ex.Message}", isError: true);
                return;
            }
            ...
            if (result.Success)
            {
                ...
                notificationManager.ShowJobSuccess(job.Name, isManual);
                return;
            }
